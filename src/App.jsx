...
export default function App() {
  ...
  const [liveLogs, setLiveLogs] = useState(['Initializing secure audit gateway...']);
        ...
        <div className="lg:col-span-1 bg-[#111827] border border-gray-800 rounded-xl p-5 flex flex-col gap-4">
          ...
          {activeTab === 'network' ? (
            <form onSubmit={runNetworkScan} className="space-y-4">
              <input type="url" value={targetUrl} onChange={(e) => setTargetUrl(e.target.value)} placeholder="https://example.com" className="w-full bg-[#0a0f1d] border border-gray-700 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 text-gray-200" />
              <button type="submit" disabled={isScanning} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg py-3 font-medium text-sm transition-colors flex items-center justify-center gap-2">
            ...
            <form onSubmit={runCodeAnalysis} className="space-y-4 flex-1 flex flex-col">
              ...
              <textarea value={codeSnippet} onChange={(e) => setCodeSnippet(e.target.value)} placeholder="// Paste code..." className="w-full flex-1 min-h-[250px] bg-[#0a0f1d] border border-gray-700 rounded-lg p-4 font-mono text-xs text-gray-200 resize-none" />
              <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg py-3 font-medium text-sm">Execute Static Audit</button>
            </form>
          )}
        </div>
      </main>
